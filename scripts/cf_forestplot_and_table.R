...
## if different number of samples
## need to match length
...
xx <-
  cbind(
    as.data.frame(cf_global) %>%
      setNames(paste("cf global", distn_tx)),
    as.data.frame(cf_os) %>%
      setNames(paste("cf os", distn_tx)),
    as.data.frame(cf_pfs) %>%
      setNames(paste("cf pfs", distn_tx)))
...
## separate plots for each cf type
#
# as.data.frame(cf_global) %>%
#   setNames(distn_tx) %>%
#   mcmc_intervals() #+
