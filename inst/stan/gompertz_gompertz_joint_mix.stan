// gompertz-gompertz mixture cure model
...
data {
  ...
  real<lower=0> a_alpha_os;
  real<lower=0> b_alpha_os;
...
model {
  ...
  for (i in 1:n_os) {
    target += log_sum_exp(
                ...
                log1m(curefrac) +
                joint_exp_gompertz_lpdf(t_os[i] | d_os[i], alpha_os, lambda_os[i], lambda_os_bg[i])) +
              log_sum_exp(
                log(curefrac) +
                surv_exp_lpdf(t_pfs[i] | d_pfs[i], lambda_pfs_bg[i]),
                log1m(curefrac) +
                joint_exp_gompertz_lpdf(t_pfs[i] | d_pfs[i], alpha_pfs, lambda_pfs[i], lambda_pfs_bg[i]));
  }
}
...
generated quantities {
  ...
  real palpha_os = gamma_rng(a_alpha_os, b_alpha_os);
