...
mean_1 = exp(beta_1[1]);
 mean_2 = exp(beta_2[1]);// background rate
           ...
           // mean_bg = 0.001;
mean_bg = mean(h_bg_1);mean_bg = mean(h_bg_2);
}
...
// likelihood
  ...
  for (i in idx_1:(idx_1 + n_1[Tx] - 1)) {
     log_lik_1[i] = log_sum_exp(
      ...
      surv_exp_lpdf(t_1[i] | d_1[i], lambda_1_bg[i]),
      log1m(cf_os[Tx]) +
      joint_exp_exponential_lpdf(t_1[i] | d_1[i], lambda_os[i], lambda_1_bg[i]));
    }
 ...
 // likelihood
  ...
  for (i in idx_2:(idx_2 + n_2[Tx] - 1)) {
     log_lik_2[i] = log_sum_exp(
      ...
      surv_exp_lpdf(t_2[i] | d_2[i], lambda_2_bg[i]),
      log1m(cf_os[Tx]) +
      joint_exp_exponential_lpdf(t_2[i] | d_2[i], lambda_os[i], lambda_2_bg[i]));
    }
...
int<lower=1, upper=2> bg_model;
        ...
        vector<lower=0>[bg_model == 1 ? H_1 : 0] sigma_bg;
vector<lower=0>[bg_model == 2 ? N_2 : 0] h_bg_2;vector[cf_model == 3 ? nTx : 0] mu_alpha;             // treatment regression coefficients
...
int<lower=1, upper=2> bg_model;
        ...
        vector<lower=0>[bg_model == 1 ? H_1 : 0] sigma_bg;
vector<lower=0>[bg_model == 2 ? N_1 : 0] h_bg_1;vector<lower=0>[cf_model == 3 ? nTx : 0] sigma_alpha;
vector[cf_model == 2 ? nTx : 0] mu_alpha_1;
vector<lower=0>[cf_model == 2 ? nTx : 0] sigma_alpha_1;
 real<lower=0> a_cf[cf_model == 1 ? 1 : 0];
real<lower=0> b_cf[cf_model == 1 ? 1 : 0];
...
int<lower=1, upper=2> bg_model;
        ...
        vector<lower=0>[bg_model == 1 ? H_1 : 0] sigma_bg;
vector<lower=0>[bg_model == 2 ? N_2 : 0] h_bg_2;int<lower=0> t_max;
vector[cf_model == 2 ? nTx : 0] mu_alpha_2;
vector<lower=0>[cf_model == 2 ? nTx : 0] sigma_alpha_2;
 real<lower=0> a_cf[cf_model == 1 ? 1 : 0];
real<lower=0> b_cf[cf_model == 1 ? 1 : 0];
