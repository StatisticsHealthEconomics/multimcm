  ...
  for (Tx in 1:nTx) {
    for (i in idx_3:(idx_3 + n_3[Tx] - 1)) {
      ...
      target += log_sum_exp(
        ...
        log1m(cf_3[Tx]) +
        joint_exp_gompertz_lpdf(t_3[i] | d_3[i], shape_3, lambda_3[i], lambda_3_bg[i]));
    }
...
generated quantities {
  ...
  for (j in 1:nTx) {
    for (i in 1:t_max) {
      S_bg[i] = exp_Surv(i, mean_bg);
      S_2[i] = exp_weibull_Surv(i, shape_{id}, mean_{id}, mean_bg);
  ...
  for (j in 1:nTx) {
    for (i in 1:t_max) {
      S_bg[i] = exp_Surv(i, mean_bg);
      S_3[i] = exp_gompertz_Surv(i, shape_{id}, mean_{id}, mean_bg);
  ...
  // for (i in 1:t_max) {
    ...
    // for (i in 1:t_max) {
      //  pS_bg[i] = exp_Surv(i, pmean_bg);
      //  pS_2[i] = exp_weibull_Surv(i, pshape_{id}, pmean_{id}, pmean_bg);
      ...
      // for (i in 1:t_max) {
        //  pS_bg[i] = exp_Surv(i, pmean_bg);
        //  pS_3[i] = exp_gompertz_Surv(i, pshape_{id}, pmean_{id}, pmean_bg);
        //  S_3_prior[i] = pmean_cf_3*pS_bg[i] + (1 - pmean_cf_3)*pS_3[i];
        // }
        // likelihood
        idx_1 = 1;
        ...
        for (Tx in 1:nTx) {
          ...
          for (i in idx_2:(idx_2 + n_2[Tx] - 1)) {
            log_lik_2[i] = log_sum_exp(
              ...
              surv_exp_lpdf(t_2[i] | d_2[i], lambda_2_bg[i]),
              log1m(cf_2[Tx]) +
              joint_exp_weibull_lpdf(t_2[i] | d_2[i], shape_{id}, lambda_{id}[i], lambda_2_bg[i]));
          }
        ...
        for (Tx in 1:nTx) {
          ...
          for (i in idx_3:(idx_3 + n_3[Tx] - 1)) {
            log_lik_3[i] = log_sum_exp(
              log(cf_3[Tx]) +
              surv_exp_lpdf(t_3[i] | d_3[i], lambda_3_bg[i]),
              log1m(cf_3[Tx]) +
              joint_exp_gompertz_lpdf(t_3[i] | d_3[i], shape_{id}, lambda_{id}[i], lambda_3_bg[i]));
          }
