  ...
  for (i in 1:n_os) {
    target += log_sum_exp(
              ...
              log_sum_exp(
                ...
                log1m(cf_pfs) +
                joint_exp_gompertz_lpdf(t_pfs[i] | d_pfs[i], shape_pfs, lambda_pfs[i], lambda_pfs_bg[i]));
  }
}
...
generated quantities {
  ...
  for (n in 1:n_os) {
    log_lik[n] = log_sum_exp(
                  ...
                  log1m(cf_os) +
                    joint_exp_gompertz_lpdf(t_os[n] | d_os[n], shape_os, lambda_os[n], lambda_os_bg[n])) +
                log_sum_exp(
                  log(cf_pfs) +
                    surv_exp_lpdf(t_pfs[n] | d_pfs[n], lambda_pfs_bg[n]),
                  log1m(cf_pfs) +
                    joint_exp_gompertz_lpdf(t_pfs[n] | d_pfs[n], shape_pfs, lambda_pfs[n], lambda_pfs_bg[n]));
  }
