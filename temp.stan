...
//     log1m(cf_2[Tx]) +
//       exp_log_S(t_2[i], lambda_2_bg[i]) + loglogistic_log_S(t_2[i], shape_2, lambda_2[i]));
//
//      target += d_2[i] * log_sum_exp(
//     log(lambda_2_bg[i]),
//     log1m(cf_2[Tx]) +
//       loglogistic_lpdf(t_2[i] | shape_2, lambda_2[i]) - log(cf_2[Tx] + (1 - cf_2[Tx])*loglogistic_Surv(t_2[i], shape_2, lambda_2[i])));
//   }
//
//   idx_2 = idx_2 + n_2[Tx];
// }
...
//   for (i in 1:t_max) {
//     S_bg[i] = exp_Surv(i, mean_bg);
//     S_2[i] = exp_loglogistic_Surv(i, shape_2, mean_2, mean_bg);
...
// // for (i in 1:t_max) {
// //  pS_bg[i] = exp_Surv(i, pmean_bg);
// //  pS_2[i] = exp_loglogistic_Surv(i, shape_2, lambda_2[j], pmean_bg);
...
//       log1m(cf_2[Tx]) +
//       joint_exp_loglogistic_lpdf(t_2[i] | d_2[i], shape_2, lambda_2[i], lambda_2_bg[i]));
//     }
//
//     idx_2 = idx_2 + n_2[Tx];
//   }
