...
for (Tx in 1:nTx) {
  for (i in idx_3:(idx_3 + n_3[Tx] - 1)) {
    ...
    log1m(cf_3[Tx]) +
      joint_exp_gengamma_lpdf(t_3[i] | d_3[i], mu_3[i], scale_3, Q_3, lambda_3_bg[i]));
  }
...
for (j in 1:nTx) {
  for (i in 1:t_max) {
    S_bg[i] = exp_Surv(i, mean_bg);
    S_3[i] = exp_gengamma_Surv(i, mean_3, scale_3, Q_3, mean_bg);
...
// for (i in 1:t_max) {
//  pS_bg[i] = exp_Surv(i, pmean_bg);
//  pS_3[i] = exp_gengamma_Surv(i, mu_3, scale_3, Q_3[j], pmean_bg);
